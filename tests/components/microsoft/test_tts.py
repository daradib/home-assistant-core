"""Tests for the Microsoft Text-to-Speech."""
import os
import shutil
...
async def test_service_say_persian_config(hass: HomeAssistant, mock_tts, calls) -> None:
...
async def test_service_say_persian_service(
    hass: HomeAssistant, mock_tts, calls
) -> None:
...
async def test_service_say_error(hass: HomeAssistant, mock_tts, calls) -> None:
    ...
    mock_tts.return_value.speak.side_effect = HTTPError
    await async_setup_component(
        hass, tts.DOMAIN, {tts.DOMAIN: {"platform": "microsoft", "api_key": ""}}
    )
